...
public class TradingService
{
    ...
    public IResult PostTrade(PostTradeRequest req)
    {
        ...
        try
        {
            ...
            var ohlc = GetOHLC(conn, tickerId, gameDate, tx);
            if (ohlc is null)
            {
                tx.Rollback();
            ...
            {
                if (gamePhase == "pre_market")
                {
                    var prevClose = GetPreviousClosePrice(conn, tickerId, gameDate, tx);
                    if (prevClose is null)
                    {
                        tx.Rollback();
                        return Results.Json(new ErrorResponse("error", $"No previous close price for {tickerId} before {gameDate}"), statusCode: 400);
                    }
